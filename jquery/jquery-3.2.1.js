...
function getDefaultDisplay( elem ) {
	var temp,
		doc = elem.ownerDocument,  //The Node.ownerDocument read-only property returns the top-level document object for this node.  当前元素的祖先
		nodeName = elem.nodeName,  
		display = defaultDisplayMap[ nodeName ]; //这里就是undefined

	if ( display ) {
		return display;
	}

	temp = doc.body.appendChild( doc.createElement( nodeName ) ); //创建子元素
	display = jQuery.css( temp, "display" ); //最终还是要找到原来的默认的display对应的值
